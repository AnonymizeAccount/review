...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, loc_dlabels_matrix):
        ...
        if self.args.model == 'mlp':
            self.enc = MLP(input_dim=(self.args.edge_dim * 2 + self.args.direction_dim) * self.args.obs_len,
                           hidden_dim=self.args.hidden_dim).to(self.args.device)
        elif self.args.model == 'rnn':
            self.enc = RNNCell(input_dim=self.args.edge_dim * 2 + self.args.direction_dim, hidden_dim=self.args.hidden_dim).to(self.args.device)
        elif self.args.model == 'gru':
            self.enc = GRUCell(input_dim=self.args.edge_dim * 2 + self.args.direction_dim, hidden_dim=self.args.hidden_dim).to(self.args.device)
    ...
    def forward(self, inputs, directions, mask, goal, epoch=None, type=None, y=None):
        if self.args.rand:
            ...
            link_embs = link_embs[:, (inputs.shape[-1] - self.args.obs_len):, :]
            direction_embs = direction_embs[:, (inputs.shape[-1] - self.args.obs_len):, :]
