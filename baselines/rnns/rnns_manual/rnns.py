...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, loc_dlabels_matrix):
        ...
        if self.args.model == 'mlp':
            self.enc = MLP(input_dim=(self.args.edge_dim + self.args.direction_dim) * self.args.seq_len,
                           hidden_dim=self.args.hidden_dim).to(self.args.device)
        elif self.args.model == 'rnn':
            self.enc = RNNCell(input_dim=self.args.edge_dim + self.args.direction_dim, hidden_dim=self.args.hidden_dim).to(self.args.device)
        elif self.args.model == 'gru':
            self.enc = GRUCell(input_dim=self.args.edge_dim + self.args.direction_dim, hidden_dim=self.args.hidden_dim).to(self.args.device)
