...
class RNN(nn.Module):
    ...
    def forward(self, inputs, directions, mask, goal, epoch=None, type=None, y=None):
        if self.args.rand:
            ...
            # print(f'directions: {directions.shape} | max: {torch.max(directions)} | min: {torch.min(directions)}')
            # assert 1==2
            ...
            # print(f'link_embs: {link_embs.shape} | direction_embs: {direction_embs.shape} | goal_direction_embs: {goal_direction_embs.shape}')

            # goal_embs = torch.unsqueeze(self.link_emblayer(goal), dim=1).expand(self.args.batch_size, self.args.seq_len, -1)
