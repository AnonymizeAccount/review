...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, direction_labels, loc_direct_matrix, loc_dist_matrix, loc_dlabels_matrix, TM, edge_A):
        ...
        self.seq_emblayer = nn.Embedding(1, 64).to(self.args.device)
    ...
    def forward(self, inputs, directions, mask):
        ...
        for i in range(self.args.seq_len - 1):
            if i == 0:
                seq_relation_input = inputs[:, :2]
            else:
                seq_relation_input = torch.cat((seq_relation_input, inputs[:, i:i+2]), dim=0)
        # print(f'seq_relation_input: {seq_relation_input.shape}')
        seq_embs = self.link_emblayer(seq_relation_input)
        # print(f'seq_embs: {seq_embs.shape}')

        seq_score_link = torch.matmul((torch.squeeze(seq_embs[:, :1]) + self.seq_emblayer.weight), self.link_emblayer.weight[1:, :].T)
        # print(f'seq_embs[:, :1]: {torch.squeeze(seq_embs[:, :1]).shape}')
        # print(f'self.seq_emblayer.weight: {self.seq_emblayer.weight.shape}')
        # print(f'self.link_emblayer.weight[1:, :].T: {self.link_emblayer.weight[1:, :].T.shape}')
        # print(f'seq_score_link: {seq_score_link.shape}')
        # print(f'seq_embs[:, 1]: {seq_embs[:, 1].shape}')
        loss += self.criterion(seq_score_link, seq_relation_input[:, 1] - 1)
        # print(loss)
        # assert 1==2
