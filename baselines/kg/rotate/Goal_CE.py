...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, direction_labels, loc_direct_matrix, loc_dist_matrix, loc_dlabels_matrix, TM, edge_A,
        ...
        self.args.edge_dim *= 2
        # self.args.direction_dim *= 2
    ...
    def forward(self, inputs, directions, mask, goal, epoch=None, type=None, y=None):
        ...
        re_head, im_head = torch.chunk(link_embs, 2, dim=1)
        # print(f're_head: {re_head.shape}')
        # print(f'im_head: {im_head.shape}')

        re_relation = torch.cos(direction_embs)
        im_relation = torch.sin(direction_embs)
        # print(f're_relation: {re_relation.shape}')
        # print(f'im_relation: {im_relation.shape}')

        re_score = re_head * re_relation - im_head * im_relation
        im_score = re_head * im_relation + im_head * re_relation
        # print(f're_score: {re_score.shape}')
        # print(f'im_score: {im_score.shape}')
        re_tail, im_tail = torch.chunk(self.link_emblayer.weight[1:, :], 2, dim=1)
        # print(f're_tail: {re_tail.shape}')
        # print(f'im_tail: {im_tail.shape}')
        re_sim_score_link = torch.matmul(re_score, re_tail.T)
        im_sim_score_link = torch.matmul(im_score, im_tail.T)
        score = torch.stack([re_sim_score_link, im_sim_score_link], dim=0)
        sim_score_link = score.norm(dim=0)

        # re_score, im_score = torch.unsqueeze(re_score, dim=1).expand(self.args.batch_size, re_tail.shape[0], re_tail.shape[1]), \
        #                      torch.unsqueeze(im_score, dim=1).expand(self.args.batch_size, im_tail.shape[0], im_tail.shape[1])
        # re_tail, im_tail = torch.unsqueeze(re_tail, dim=0), torch.unsqueeze(im_tail, dim=0)
        # # print(f're_score: {re_score.shape}')
        # # print(f'im_score: {im_score.shape}')
        # # print(f're_tail: {re_tail.shape}')
        # # print(f'im_tail: {im_tail.shape}')
        ...
        #
        # sim_score_link = score.norm(dim=0).sum(dim=2)
