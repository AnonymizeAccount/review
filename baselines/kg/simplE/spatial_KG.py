...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, direction_labels, loc_direct_matrix, loc_dist_matrix, loc_dlabels_matrix, TM, edge_A,
        ...
        # self.edge_ids = torch.arange(self.args.num_edges).to(self.args.device)
        ...
        self.distance_inv_emblayer = nn.Embedding(self.args.pre_len, self.args.distance_dim).to(self.args.device)
        self.connect_inv_emblayer = nn.Embedding(1, self.args.connection_dim).to(self.args.device)
    ...
    def forward(self, inputs, directions, mask, goal, epoch=None, type=None, y=None):
        ...
        # (batch_size, edge_dim)
        link_hh_embs = self.link_h_emblayer(inputs[:, -1])
        link_th_embs = self.link_t_emblayer(inputs[:, -1])
        # (batch_size, edge_dim)
        direction_embs = self.direction_emblayer(goal_directions)
        direction_inv_embs = self.direction_inv_emblayer(goal_directions)

        # connection
        heads, tails = torch.where(self.edge_A == 1)
        ids = torch.randperm(heads.shape[0])[:self.args.batch_size].to(self.args.device)
        heads, tails = heads[ids], tails[ids]
        s_connection_hh_link_embs = self.link_h_emblayer(heads + 1)
        s_connection_ht_link_embs = self.link_h_emblayer(tails + 1)
        # e_connection_th_link_embs = self.link_t_emblayer(heads + 1)
        # e_connection_tt_link_embs = self.link_t_emblayer(tails + 1)
        connection_embs = self.connect_emblayer.weight.expand(s_connection_hh_link_embs.shape[0], -1)
        connection_inv_embs = self.connect_inv_emblayer.weight.expand(s_connection_hh_link_embs.shape[0], -1)
        # print(f's_connection_hh_link_embs: {s_connection_hh_link_embs.shape}')
        # print(f's_connection_th_link_embs: {s_connection_ht_link_embs.shape}')
        # print(f'connection_embs: {connection_embs.shape}')
        # print(f'connection_inv_embs: {connection_inv_embs.shape}')
        # print(f'link_t_emblayer: {self.link_t_emblayer.weight[1:, :].shape}')
        score_1 = s_connection_hh_link_embs * connection_embs @ self.link_t_emblayer.weight[1:, :].T
        score_2 = s_connection_ht_link_embs * connection_inv_embs @ self.link_t_emblayer.weight[1:, :].T
        # print(f'score_1: {score_1.shape}')
        # print(f'score_2: {score_2.shape}')
        loss += self.args.a * \
                (self.criterion(torch.log_softmax(score_1, dim=1), tails) +
                 self.criterion(torch.log_softmax(score_2, dim=1), heads)) / 2

        # direction
        heads, tails = torch.randperm(self.args.num_edges)[:self.args.batch_size].to(self.args.device), \
                       torch.randperm(self.args.num_edges)[:self.args.batch_size].to(self.args.device)
        sampled_directions_embs = self.direction_emblayer(self.loc_dlabels_matrix[heads, tails] + 1)
        direction_s_link_embs = self.link_emblayer(heads + 1)
