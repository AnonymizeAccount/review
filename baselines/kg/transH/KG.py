...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, direction_labels, loc_direct_matrix, loc_dist_matrix, loc_dlabels_matrix, TM, edge_A,
        ...
        self.rank_predictor = nn.Sequential(
            nn.Linear(self.args.multi ** self.args.pre_len * self.args.hidden_dim * 3, self.args.hidden_dim),
            nn.ReLU(),
    ...
    def compute_loss(self, pred, gt, pred_d, gt_d):
        ...
        for dim in range(gt.shape[1]):
            loss += self.args.e * self.criterion(pred[:, dim * self.args.num_edges: (dim + 1) * self.args.num_edges], gt[:, dim])
        return loss
    ...
    def rerank(self, preds_topk, gt):
        ...
        return self.args.d * loss_ranking, preds_topk.permute((0, 2, 1))
