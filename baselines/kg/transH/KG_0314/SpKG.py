...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, direction_labels, loc_direct_matrix, loc_dist_matrix, loc_dlabels_matrix, TM, edge_A,
        ...
        self.hidden_states = torch.zeros(self.args.batch_size * args.multi**args.pre_len, self.args.hidden_dim).to(self.args.device)
        self.cell_states = torch.zeros(self.args.batch_size * args.multi**args.pre_len, self.args.hidden_dim).to(self.args.device)

        self.rank_predictor = nn.Sequential(
            nn.Linear(self.args.multi ** self.args.pre_len * (self.args.hidden_dim + self.args.connection_dim + self.args.consistent_dim),
    ...
    def forward(self, inputs, directions, mask, goal, epoch=None, type=None, y=None):
        ...
        return pred_hard, self.pred_d_rand, loss, direction_correct