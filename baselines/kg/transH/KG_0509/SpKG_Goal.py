...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, direction_labels, loc_direct_matrix, loc_dist_matrix, loc_dlabels_matrix, TM, edge_A,
        ...
        print(f'Model: spatialKG_rerank_1_best_0314_SpKG_GOAL')
        self.args = args
        ...
        # self.mlp = MLP(input_dim=self.args.edge_dim * 2 + self.args.direction_dim * 2, hidden_dim=self.args.hidden_dim).to(self.args.device)
        # self.out_link = nn.Linear(self.args.hidden_dim, self.args.num_edges * self.args.pre_len).to(self.args.device)
        # self.topk_compress = nn.Linear((self.args.multi ** self.args.pre_len) * self.args.pre_len * (self.args.edge_dim + self.args.direction_dim),
        #                                self.args.edge_dim * 2 + self.args.direction_dim * 2)
    ...
    def compute_loss(self, pred, gt, pred_d, gt_d):
        ...
        for dim in range(gt.shape[1]):
            loss += self.args.f * self.criterion(pred[:, dim * self.args.num_edges: (dim + 1) * self.args.num_edges], gt[:, dim])
        return loss
    ...
    def rerank(self, preds_topk, gt, type=None):
        ...
        # print(f'preds_topk_embs | max: {torch.max(preds_topk_embs)} | min: {torch.min(preds_topk_embs)} | {preds_topk_embs.shape}')
    ...
    def forward(self, inputs, directions, mask, goal, epoch=None, type=None, y=None):
        ...
        pred_hard, pred_hard_detached = None, None

        # print(f'inputs: {inputs.shape} | max: {torch.max(inputs)} | min: {torch.min(inputs)}')
        # assert 1==2
        ...
        # print(f'd_embs: {d_embs.shape} | link_embs: {link_embs.shape} | goal_embs: {goal_embs.shape} | direction_embs: {direction_embs.shape}')
        embs = torch.cat((link_embs, direction_embs, goal_embs, d_embs), dim=-1)

        # mlp_pred = self.out_link(self.mlp(embs))
        ...
        for i, id in enumerate(self.pre_len_ids):
            ...
            # print(f'sim_score_link: {sim_score_link.shape} | mlp_out: {mlp_out.shape} | num_edges: {self.args.num_edges}')
            # loss += 0.1 * F.kl_div((F.softmax(sim_score_link, dim=-1) + 1e-8).log(),
            #                  F.softmax(teacher_logits[:, i*self.args.num_edges: (i+1)*self.args.num_edges], dim=-1), reduction='batchmean')
            # print(loss)
            if i == 0:
        ...
        return pred_hard, pred_d_rand, loss, direction_correct, embs