...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, direction_labels, loc_direct_matrix, loc_dist_matrix, loc_dlabels_matrix, TM, edge_A,
        ...
        if args.goal:
            ...
            print(f'Model: spatialKG_rerank_1_best_0314_SpKG')
        self.args = args
        ...
        if self.args.goal:
            self.mlp = MLP_model(input_dim=(self.args.edge_dim * 2 + self.args.direction_dim * 2 + self.args.hidden_dim),
                                 args=self.args).to(args.device)
        else:
            self.mlp = MLP_model(input_dim=(self.args.edge_dim + self.args.direction_dim * 2 + self.args.hidden_dim),
                                 args=self.args).to(args.device)
