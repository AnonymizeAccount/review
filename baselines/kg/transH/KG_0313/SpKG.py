...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, direction_labels, loc_direct_matrix, loc_dist_matrix, loc_dlabels_matrix, TM, edge_A,
        ...
        print(f'Model: spatialKG_rerank_1_best_0312_SpKG')
        self.args = args
        ...
        # self.loc_nxdist_matrix = torch.tensor(loc_nxdist_matrix).to(self.args.device)
    ...
    def forward(self, inputs, directions, mask, goal, epoch=None, type=None, y=None):
        ...
        if type == 'train':
            ...
            # Direction Relation
            # Positive Direction
            samples = inputs[:, self.args.pre_len:] - 1
            idx = torch.randperm(self.args.pre_len).to(self.args.device)
            rows, cols = samples[:, idx[0]], samples[:, idx[1]]
            dist = self.lambda_distance(torch.abs(idx[0]-idx[1]))
            ...
            sampled_direction_tail_embs = direction_tail_embs[sampled_directions - 1, :, :]
            direction_heads = torch.unsqueeze((direction_heads + sampled_directions_embs) * dist, dim=1)
            # print(f'direction_heads: {direction_heads.shape}')
            sim_score_link = torch.sum(direction_heads * sampled_direction_tail_embs, dim=-1)
            # print(f'sim_score_link: {sim_score_link.shape}')
            loss += self.args.b * self.criterion(sim_score_link, cols)
