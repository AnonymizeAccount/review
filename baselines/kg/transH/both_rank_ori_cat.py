...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, direction_labels, loc_direct_matrix, loc_dist_matrix, loc_dlabels_matrix, TM, edge_A,
        ...
        print(f'Model: spatialKG with ranklayers and original emblayers, cat')
        self.args = args
        ...
        self.rank = LSTMCell(input_dim=(self.args.edge_dim + self.args.direction_dim) * 2,
                             hidden_dim=self.args.hidden_dim).to(self.args.device)
    ...
    def rerank(self, preds_topk, gt):
        ...
        preds_topk_embs = torch.cat((preds_topk_embs, self.link_emblayer(preds_topk).detach()), dim=-1)
        ...
        preds_topk_d_embs = torch.cat((preds_topk_d_embs, self.direction_emblayer(preds_topk_d_labels).detach()), dim=-1)
        # (batch_size * topk, pre_len, edge_dim + direction_dim)
        preds_topk_embs = torch.cat((preds_topk_embs, preds_topk_d_embs), dim=-1)\
            .reshape(-1, self.args.pre_len, (self.args.edge_dim + self.args.direction_dim)*2)
