...
class MLP_rank(nn.Module):
    def __init__(self, input_dim, args):
        ...
        self.fc2 = nn.Linear(self.args.hidden_dim, self.args.hidden_dim)
        self.fc3 = nn.Linear(self.args.hidden_dim, self.args.hidden_dim)
        self.out = nn.Linear(self.args.hidden_dim, self.args.hidden_dim * self.args.multi ** self.args.pre_len).to(self.args.device)
...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, direction_labels, loc_direct_matrix, loc_dist_matrix, loc_dlabels_matrix, TM, edge_A,
        ...
        self.mlp = MLP_model(input_dim=(self.args.edge_dim * 2 + self.args.direction_dim * 2 + self.args.hidden_dim), args=self.args).to(args.device)
        ...
        self.rank = MLP_rank(input_dim=self.args.multi ** self.args.pre_len * self.args.pre_len * (self.args.edge_dim + self.args.direction_dim),
    ...
    def forward(self, inputs, directions, mask, goal, epoch=None, type=None, y=None):
        ...
        self.embs = torch.cat((link_embs, direction_embs, goal_embs, d_embs), dim=-1)
