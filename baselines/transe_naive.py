...
class RNN(nn.Module):
    def __init__(self, args, graph, node_adj_edges, nodes_links_directions, direction_labels, loc_direct_matrix, loc_dist_matrix, loc_dlabels_matrix):
        ...
        # self.distfn = nn.PairwiseDistance(p=2)
    ...
    #         # print(f'sample_o: {sample_o.shape}\n{sample_o}')
    #         # print(f'sample_d: {sample_d.shape}\n{sample_d}')
    ...
    #         # print(f'head: {head_entities.shape}')
    #         # print(f'tail: {tail_entities.shape}')
    #         # print(f'relation: {relations.shape}')
    #         # print(f'self.link_emblayer.weight[1:, :].T: {self.link_emblayer.weight[1:, :].T.shape}')
    #
    #         # loss = self.distfn(head_entities+relations, tail_entities)
    #         kg_sim_score_link = torch.matmul((head_entities+relations), self.link_emblayer.weight[1:, :].T)
    #         # print(f'kg_sim_score_link: {kg_sim_score_link.shape}')
    ...
    #         # assert 1==2
    #
    #         # return pred, pred_d, loss.sum() / self.args.batch_size
    #         return pred, pred_d, loss




    # Knowledge Graph + Random Sample from X (CE)
    def forward(self, inputs, directions, mask):
        loss = 0
        if self.args.rand:
            pred = torch.rand((self.args.batch_size, self.args.num_edges * self.args.pre_len)).to(self.args.device)
    ...
    #         # print(f'direction_embs: {direction_embs.shape}')
    #
    #         assert 2 * self.args.batch_size <= self.args.num_edges
    #         sample = torch.randperm(n=self.args.num_edges, device=self.args.device)[:2*self.args.batch_size]
    #         # print(f'sample: {sample.shape}\n{sample}')
    #         # print(f'self.loc_dlabels_matrix: {self.loc_dlabels_matrix.shape}')
    #
    #         head_entities, tail_entities = self.link_emblayer(sample[:self.args.batch_size]+1), self.link_emblayer(sample[self.args.batch_size:]+1)
    #         relations = self.direction_emblayer(self.loc_dlabels_matrix[(sample[:self.args.batch_size], sample[self.args.batch_size:])]+1)
    #         # print(f'head: {head_entities.shape}\n{head_entities}')
    #         # print(f'tail: {tail_entities.shape}\n{tail_entities}')
    #         # print(f'relation: {relations.shape}\n{relations}')
    #
    #         # loss = self.distfn(head_entities+relations, tail_entities)
    #         kg_sim_score_link = torch.matmul((head_entities+relations), self.link_emblayer.weight[1:, :].T)
    #         loss = self.criterion(kg_sim_score_link, sample[self.args.batch_size:])
    #
    #         sim_score_link = torch.matmul((link_embs+direction_embs), self.link_emblayer.weight[1:, :].T)
    #         # print(f'sim_score_link: {sim_score_link.shape}')
    #         # assert 1==2
