...
def eval_one_epoch(loaders, args, G, G_edges):
    ...
    for loader in loaders:
        for i, (x, y, direction_x, direction_y, length) in enumerate(loader.get_iterator()):
            ...
            for j in range(x.shape[0]):
                ...
                for k in range(len(paths)-1):
                    ...
                    #     assert 1==2
                while len(edge_path) < 5:
                    if len(edge_path) == 0:
                        edge_path.append(1)
                    else:
                        edge_path.append(edge_path[0])
                ...
                if pred is None:
                    pred = torch.unsqueeze(torch.from_numpy(edge_path), dim=0)
                else:
                    pred = torch.cat((pred, torch.unsqueeze(torch.from_numpy(edge_path), dim=0)), dim=0)
        ...
        # end_time = time.time()
    # print(f'all_route: {all_route} | all_traj: {all_traj}')
    ...
    print(f'MAE: {mae:.3f} | RMSE: {rmse:.3f} | r2: {r_squared:.3f}')
