...
from .new_kg_epoch import train_one_epoch, eval_one_epoch
...
def train(loader, args, model, opt_model):
    ...
    for ep in range(args.epoch):
        ...
        print(f'Ep {ep + 1} | p:{count_patience} | Loss:{loss:.2f} | T: {train_acc:.3f}, {train_traj_acc:.3f}, d: {train_goal_d_acc:.3f} | '
              f'V: [{val_acc[0]:.3f}, {val_acc[1]:.3f}, {val_acc[2]:.3f}, {val_acc[3]:.3f}, {val_acc[4]:.3f}] '
              f'[{val_traj_acc[0]:.3f}, {val_traj_acc[1]:.3f}, {val_traj_acc[2]:.3f}, {val_traj_acc[3]:.3f}, {val_traj_acc[4]:.3f}], d: {val_goal_d_acc:.3f} | '
              f'T: [{test_acc[0]:.3f}, {test_acc[1]:.3f}, {test_acc[2]:.3f}, {test_acc[3]:.3f}, {test_acc[4]:.3f}] '
              f'[{test_traj_acc[0]:.3f}, {test_traj_acc[1]:.3f}, {test_traj_acc[2]:.3f}, {test_traj_acc[3]:.3f}, {test_traj_acc[4]:.3f}], '
              f'm: [{test_mrr[0]:.3f}, {test_mrr[1]:.3f}, {test_mrr[2]:.3f}, {test_mrr[3]:.3f}, {test_mrr[4]:.3f}], d: {test_goal_d_acc:.3f} | '
              f'Time: {dur[-1]:.2f}')
    ...
    test_acc, test_traj_acc, _, _, test_mrr = eval_one_epoch(test_loaders, args, model, opt_model, type='topk', epoch=99999)
    # print(f'Final test result: [{test_acc[0]:.3f}, {test_acc[1]:.3f}, {test_acc[2]:.3f}, {test_acc[3]:.3f}, {test_acc[4]:.3f}], '
    #       f'[{test_traj_acc[0]:.3f}, {test_traj_acc[1]:.3f}, {test_traj_acc[2]:.3f}, {test_traj_acc[3]:.3f}, {test_traj_acc[4]:.3f}], '
    #       f'm: [{test_mrr[0]:.3f}, {test_mrr[1]:.3f}, {test_mrr[2]:.3f}, {test_mrr[3]:.3f}, {test_mrr[4]:.3f}]')

    print(f'a: {args.a} | b: {args.b} | c: {args.c} | d: {args.d} | e: {args.e} | f: {args.f} | g: {args.g} | h: {args.h} | pre_len: {args.pre_len} | '
          f'[{test_acc[0]:.3f}, {test_acc[1]:.3f}, {test_acc[2]:.3f}, {test_acc[3]:.3f}, {test_acc[4]:.3f}], '
          f'[{test_traj_acc[0]:.3f}, {test_traj_acc[1]:.3f}, {test_traj_acc[2]:.3f}, {test_traj_acc[3]:.3f}, {test_traj_acc[4]:.3f}], '
          f'm: [{test_mrr[0]:.3f}, {test_mrr[1]:.3f}, {test_mrr[2]:.3f}, {test_mrr[3]:.3f}, {test_mrr[4]:.3f}]')

    fo = open(f"{args.result_dir}", "a")
    fo.write(f'a: {args.a} | b: {args.b} | c: {args.c} | d: {args.d} | e: {args.e} | f: {args.f} | g: {args.g} | h: {args.h} | pre_len: {args.pre_len} |'
             f'[{test_acc[0]:.3f}, {test_acc[1]:.3f}, {test_acc[2]:.3f}, {test_acc[3]:.3f}, {test_acc[4]:.3f}], '
             f'[{test_traj_acc[0]:.3f}, {test_traj_acc[1]:.3f}, {test_traj_acc[2]:.3f}, {test_traj_acc[3]:.3f}, {test_traj_acc[4]:.3f}], '
             f'm: [{test_mrr[0]:.3f}, {test_mrr[1]:.3f}, {test_mrr[2]:.3f}, {test_mrr[3]:.3f}, {test_mrr[4]:.3f}]\n')
    fo.close()
    ...
    if args.save:
        ...
        if args.goal:
            save_dir = os.path.join(args.base_dir, 'checkpoints', args.dataset, f'goal.pkl')
        elif args.ed:
            save_dir = os.path.join(args.base_dir, 'checkpoints', args.dataset, f'estimate_goald.pkl')
        else:
            save_dir = os.path.join(args.base_dir, 'checkpoints', args.dataset, f'goal_d.pkl')

        torch.save(state, save_dir)

    # state = torch.load(save_dir, map_location='cpu')
