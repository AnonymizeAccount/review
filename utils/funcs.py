...
def get_multi_predictions(pred, pred_d, gt, direction_gt, args, graph_edges, all_node_adj_edges, ids, offset, padding_loglikelihood, obs=None):
    ...
    for dim in range(args.pre_len):
        ...
        # print(f'12: {time.time() - start_12}')
        # assert 1==2

    walker = Walker()
    # print(f'1: {time.time() - start_1:.3f}')
    ...
    # print(f'2: {time.time() - start_2:.3f}')
    # assert 1==2
...
# def get_end_nodes_RCM(pred, dim, end_node, cur_parent, args, padding_loglikelihood, graph_edges, ids, offset, all_node_adj_edges):
#     # (batch_size, num_directions)
#     cur_state_action = torch.index_select(args.state_action, dim=0, index=cur_parent.pred)
#     # (batch_size, num_edges)
#     cur_pred = F.log_softmax(pred[:, dim * args.num_edges: (dim + 1) * args.num_edges], dim=1)
#     # (batch_size, num_edges + 1), self.padding_loglikelihood == -inf
#     cur_pred = torch.cat((cur_pred, padding_loglikelihood), dim=-1)
...
#     node_adj_edges[torch.where(last_pred == node_adj_edges)] = args.num_edges
#
#     # I think we should do some masking here
#     # print(f'node_adj_edges: {node_adj_edges.shape}\n{node_adj_edges}')
#     # print(f'cur_state_action: {cur_state_action.shape}\n{cur_state_action}')
#     cur_state_action = torch.sort(cur_state_action, dim=1)[0][:, :node_adj_edges.shape[1]]
#     # print(f'cur_state_action: \n{cur_state_action}')
#     if torch.sum((cur_state_action != node_adj_edges) * 1).item() > 0:
#         print(torch.sum((cur_state_action != node_adj_edges) * 1))
#         assert 1==2
#
#
#     # (batch_size, num_edges + 1) -> (batch_size, max_adj_edges), values: log_likelihood
...
#
#
# def get_multi_predictions_RCM(pred, pred_d, gt, direction_gt, args, graph_edges, all_node_adj_edges, ids, offset, padding_loglikelihood, obs=None):
#
#     # (batch_size), indicating the end_node of the last observed edge, which is also the start_node of the first gt edges. value: 0 ~ num_nodes-1
#     end_node = graph_edges[gt[:, 0]][:, 0] - 1
#     prediction_d = None
...
#     # print(f'args.state_action: {args.state_action.shape}\n{args.state_action}')
#     # selected_rows = torch.index_select(args.state_action, dim=0, index=obs)
#     # print(f'selected_rows: {selected_rows.shape}\n{selected_rows}')
#     # assert 1==2
#
#     root = Node(name="root", data=end_node, pred=obs)
#     for dim in range(args.pre_len):
...
#             cur_parent = node
#             end_node = node.data
#             get_end_nodes_RCM(pred, dim, end_node, cur_parent, args, padding_loglikelihood, graph_edges, ids, offset, all_node_adj_edges)
#     walker = Walker()
#
#     preds, k = None, 0
#     if args.topk < 0:
